...
function updateProductSelects() {
  const selects = ['entry-product', 'exit-product', 'transfer-product'];
  ...
  selects.forEach(selectId => {
    ...
    products.forEach(p => {
      ...
      option.value = p.id;
      option.textContent = `${p.code} - ${p.name} (Stock: ${p.stock})`;
...
document.getElementById('entry-form').addEventListener('submit', async (e) => {
  ...
  const reason = document.getElementById('entry-reason').value || 'Entrada';
...
document.getElementById('exit-form').addEventListener('submit', async (e) => {
  ...
  const productId = document.getElementById('exit-product').value;
  const product = products.find(p => p.id === productId);
  if (!product) return;
  ...
  const reason = document.getElementById('exit-reason').value || 'Salida';
